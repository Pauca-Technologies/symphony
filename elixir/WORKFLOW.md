...
- If useful work can continue only after GitHub Actions recovers, PR checks change, a git ref advances, or Linear activity changes, call Symphony's `wait_for` tool once and end the turn. Do not repeatedly poll an unchanged external condition; Symphony will release the agent slot and resume the issue after the condition changes. Never use `wait_for` for local CPU or memory pressure, another validation running, local process or port contention, or a time delay; continue useful work and allow independently bounded validations to overlap.
