defmodule SymphonyElixir.BaseDrift do
  ...
  def assess(workspace, %Issue{} = issue, base_ref, opts)
    ...
    with {:ok, head_sha} <- value(runner, workspace, ["rev-parse", "HEAD"]),
      ...
      compute(runner, workspace, issue, base_ref, head_sha, current_base_sha)
    end
  end
  ...
  defp compute(runner, workspace, issue, base_ref, head_sha, current_base_sha) do
    with {:ok, candidate_base_sha} <- value(runner, workspace, ["merge-base", head_sha, current_base_sha]),
      ...
      overlap = RepositoryScheduler.overlap(candidate_paths, upstream_paths)
      advanced? = candidate_base_sha != current_base_sha
      defer? = advanced? and overlap.paths != []
      ...
      decision = %{
        ...
        overlap_paths_omitted: overlap.omitted,
  ...
  defp remediation(decision) do
    ...
    Symphony deferred final validation and automated review because origin/#{decision.base_ref} advanced on paths that overlap this candidate: #{format_overlap(decision)}.
