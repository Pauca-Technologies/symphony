...
- **Option B polish — tool-call lifecycle, tokens, transcript window, timestamps — ✅ DONE (2026-06-17).** Four fixes found while testing OpenCode over ACP:
  - **Tool arguments + output.** ACP sends the initial `tool_call` with an empty `rawInput` (or, for `execute`, only `cwd`); the real arguments and the **cumulative** output stream in over later `tool_call_update`s. The renderers now key tool/output blocks on `toolCallId` and merge updates in place (text-only, so the title set by the first call survives) — so arguments fill in and cumulative output renders once instead of being concatenated into itself. (`Orchestrator.acp_tool_update_blocks/3` + `upsert_keyed_transcript_block/4`; `Presenter.acp_tool_update_fragments/3` + keyed `merge_transcript_fragments/1`; `CodexSessionLogRenderer.upsert_acp_*`.)
  - **Token totals.** `Orchestrator.extract_token_usage/1` now recognizes a flat usage map handed over as `:usage` metadata (Claude Code `result.usage`, ACP `usage_update`) and the `params.update[.usage]` paths, and synthesizes a missing total from input+output. Note: OpenCode does **not** emit any token usage over ACP in the observed sessions, so its counters stay 0 until/unless it sends `usage_update` — an upstream limitation, not a renderer bug.
  - **Transcript window.** The details page now reads the full persisted session log for running runs too (previously the capped in-memory ring buffer, which dropped older entries as new ones streamed in); disk read limits raised accordingly (`Presenter` `@transcript_{line,block,byte}_limit`).
  - **Timestamps.** The dashboard's `<time>` localizer observes `characterData` mutations so LiveView text patches no longer revert a localized time back to UTC (`Layouts.root` MutationObserver).
...
- **`run_turn/4`** — write one user-message stream-json line to stdin; read the streamed events; map the terminal `result` stop reason → `:turn_completed` / abnormal. **Option-A normalization again:** translate Claude's `assistant`/`tool_use`/`result`/partial message types into Symphony's existing `on_message` atoms (§2.2) — new mapping table, same pattern, **zero downstream changes**. *Observability polish (2026-06-17, alongside Phase 4):* the `TodoWrite` builtin normalizes to the shared `session/update`/`plan` checklist block (reusing the Phase-4 native plan rendering) instead of a raw tool-call args dump, and a failed `tool_result` (`is_error`) is prefixed `[tool error]` so failures are distinguishable from ordinary output. Both are inside `ClaudeCode.Client`; renderers unchanged.
...
- **Model (best actual signal per backend).** Codex reports the model it resolved in the top-level `thread/start` response; `Codex.AppServer` retains that value in the session and carries it on `:session_started`. For ACP and Claude Code, the initial model is the config/override value actually passed to the agent process — `session.acp.model` (→ `OPENCODE_CONFIG_CONTENT`) or `session.claude_code.model` (→ `--model`); `nil` when unset. Claude Code then reports the model it actually resolved in its `system/init`, which `ClaudeCode.Client` carries on the `:session_started` event; the orchestrator's `model_for_update/2` prefers that over the configured value. ACP emits no resolved model, so its configured value (or `nil`) stands.
